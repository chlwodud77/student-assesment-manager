...
# from model.Student import Student as st
...
def setScoreChangeAssesment(self):
    ...
    subjectBScoreArray = []
    compareSubject = None
    compareStandardArray = []
    compareAssesmentArray = []
...
def showScoreList(self, studentArray, scoreAArray, scoreBArray, standardArray, assesmentArray):
    ...
    for student, i in zip(studentArray, range(len(studentArray))):
        ...
        scoreAItem = QTableWidgetItem(str(scoreAObj.getScore())) if scoreAObj.getScore() is not None else QTableWidgetItem("")
        scoreBItem = QTableWidgetItem(str(scoreBObj.getScore())) if scoreBObj.getScore() is not None else QTableWidgetItem("")
...
def saveScoreChangeAssesment(self):
    if(scoreChangeAssesments == []): return QMessageBox.about(self,"알림", "점수변동평가를 먼저 생성해주세요.")
    for score in scoreChangeAssesments:
        ...
        if(Score.isScoreExist(subId, stdId) == True):
            result = Score.saveScore(subId, stdId, scoreNum, asses)
            if(result == True): QMessageBox.about(self,"알림", "점수변동평가 저장 완료.")
            else: return QMessageBox.about(self, "알림", "점수변동평가 저장 실패.")
        else:
            result = Score.updateScore(subId, stdId, scoreNum, asses)
            if(result == True): QMessageBox.about(self,"알림", "점수변동평가 업데이트 완료.")
            else: return QMessageBox.about(self, "알림", "점수변동평가 저장 실패.")
...
def getAssesmentFromScoreChange(changeScore, standardArray, assesmentArray):
    try:
        ...
        for standard in standardArray:
            ...
            if(greater < changeScore and changeScore <= less):
